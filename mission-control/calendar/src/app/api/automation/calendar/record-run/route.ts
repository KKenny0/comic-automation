...
const recordCronRun = makeFunctionReference<"mutation">("events:recordCronRun");

type Body = {
  cronJobId: string;
  runAt: number;
  runStatus: "success" | "failed" | "running" | "skipped";
  summary?: string;
  nextRunAt?: number;
};
...
export async function POST(req: Request) {
  try {
    ...
    if (!body?.cronJobId || !body?.runAt || !body?.runStatus) {
      return NextResponse.json({ error: "Missing required fields" }, { status: 400 });
    }

    const client = new ConvexHttpClient(convexUrl);
    const result = await client.mutation(recordCronRun, body);
